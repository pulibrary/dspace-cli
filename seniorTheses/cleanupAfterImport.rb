...
def fixNullAbstract
  ...
  items.each do |i|
    begin
      if i.archived? && (i.getParentObject.getParentObject.getHandle == $root)
        # its in a collection inside root (senior thesis)
        # check that there is exactly one abstract  - jyst to be anal
        if i.getMetadata('dc', 'description', 'abstract', nil, '*').length > 1
  ...
  items.each do |i|
    begin
      ...
      if i.archived?
        ...
        $logger.warn "#{i}  has multiple department values" if dept_vals.length > 1
