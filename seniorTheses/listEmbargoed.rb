...
def group_restricted(group_name, after_year)
  ...
  ihash = []
  fields = %w[handle collection year goup embargo group accessRights]
  ...
  mudds.each do |b|
    ...
    next unless h[klass].to_i > after_year
...
%w[SrTheses_Bitstream_Read_Mudd SrTheses_Bitstream_Read_Princeton SrTheses_Item_Read_Anonymous].each do |group_name|
