...
class Dataspace < Thor
  ...
  class SeniorTheses < Thor
    ...
    def batch_advance_workflow
      ...
      shell.say_status(:ok, "Found #{query.results.length} Items for the academic year #{year}")
      ...
      shell.say_status(:ok, "Found #{sub_query.results.length} Items for the department #{department}")
      ...
      shell.say_status(:ok, "Found #{pending.length} Items which need to be advanced to the pending editorial review workflow state")
      ...
      pending.each do |pending_item|
        ...
        (initial_state..Java::OrgDspaceWorkflow::WorkflowManager::WFSTATE_STEP3POOL).each do |new_state|
          ...
          shell.say_status(:ok, "Advancing the WorkflowItem state for #{pending_item.id} to the state #{new_state}")
        end

        pending_item.add_task_pool_user(editor_email)
        shell.say_status(:ok, "Added the Task Pool entry for user #{editor_email}")
      end
    end
